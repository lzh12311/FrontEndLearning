<template>
  <div class="page-right">
    ...
    <div v-show="setType === 2">
      <ComTitle :title="currentComponent && currentComponent.name || '组件管理'" />
      <ComValidTime v-if="currentComponent" v-model="currentComponent.data.validTime" />
...
export default {
  ...
  computed: {
    ...
    currentComponent() {
      ...
      return list && list.length > 0 ? list.find(item => item.id === this.activeComponentId) : null
    }
  },
  methods: {
