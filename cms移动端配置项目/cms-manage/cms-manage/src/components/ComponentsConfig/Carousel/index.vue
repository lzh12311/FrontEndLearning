<template>
  <div>
    ...
    <!-- 添加图片 -->
    <ComGroup title="添加图片" :tips="'最多添加'+ selectLimitSize +'个广告，鼠标拖拽调整广告顺序，建议宽度750像素'" :name-black="true" />
    ...
    <ComGroup v-show="!!configData.isDefaultMargin && configData.layout === 'single'" title="图片边距">
      <el-slider
...
export default {
  ...
  computed: {
    ...
    // 是否设置边距
    selectMarginStyle() {
      return this.radioList.find(item => item.id === this.configData.isDefaultMargin)
        .name
    },
    // 当前选择的图片样式
    // selectImageStyle() {
